    ...
    for (let r = 0; r < SIZE; r++) {
        for (let c = 0; c < SIZE; c++) {
            ...
            if (cell.conflict) {
                ...
                ctx.fillStyle = (cellBoxRow + cellBoxCol) % 2 === 0 ? '#ffffff' : '#fafafa';
            }
...
export class BoardManager {
    ...
    static operateCell(board, row, col, updateFn, options = {}) {
        const { boxSize, size, history } = options;
        ...
        if (history) {
            const result = SudokuGameHelper.saveState(
                history.historyMap, history.stepPointer, board
            );
            newHistoryMap = result.newHistoryMap;
            newStepPointer = result.newStepPointer;
        }

        updateFn(newBoard[row][col]);
