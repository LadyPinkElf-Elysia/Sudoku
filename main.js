...
const app = createApp({
    ...
    watch: { 'config.N'() { if (this.config.blanks < this.minBlanks) this.config.blanks = this.minBlanks; if (this.config.blanks > this.maxBlanks) this.config.blanks = this.maxBlanks; } },
    methods: {
        renderCanvas() {
        ...
        resetGame() { this.game.started = false; this.game.hintMessage = ''; const canvas = document.getElementById('sudokuCanvas'); if (canvas) canvas.removeEventListener('click', this.onCanvasClick); },
        saveState() { const result = SudokuGameHelper.saveState(this.historyMap, this.stepPointer, this.game.board); this.historyMap = result.newHistoryMap; this.stepPointer = result.newStepPointer; this.game.hintMessage = ''; },
        ...
        giveHint() {
            ...
            if (this.game.selectedRow === null || this.game.selectedCol === null) { this.game.hintMessage = '💡 请先在棋盘上点击选中一个空格'; return; }
        ...
        _operateCell(updateFn) {
            ...
            if (cell.conflict && this.config.mode === 'limited') {
                this.game.errors++;
                if (this.game.errors > this.config.errorLimit) { this.game.over = true; this.game.selectedRow = null; this.game.selectedCol = null; }
            }
            this.checkComplete();
            this.renderCanvas();
        },
        ...
        checkComplete() { if (SudokuGridHelper.checkComplete(this.game.board, this.SIZE)) { this.game.complete = true; this.game.selectedRow = null; this.game.selectedCol = null; this.game.hintMessage = ''; } },
